...
module.exports = {
  ...
  */
  ...
  async monitorMinedPayments () {
    ...
    try {
      ...
      console.log('monitorMinedPayments: batchMinedPayments error', e)
    }

    try {
      await TransactionHelper.checkBatchPaymentsStatus()
    } catch (e) {
      console.log('monitorMinedPayments: checkBatchPaymentsStatus error', e)
    }
  ...
  */
  async batchMinedPayments () {
    ...
    console.log('batchMinedPayments: check unbatched mined payments: ', unbatched_pmnts.length)
  ...
  async addTransactionToQueue (txType, txData) {
    ...
    console.log('addTransactionToQueue: data: ', packetData)
  ...
  async broadcastQueuedMintTransactions () {
    ...
    if (hasQueuedTransaction && !hasPendingTransaction) {
      try {
        ...
        console.log('broadcastQueuedMintTransactions: nextQueuedTransactionData', nextQueuedTransactionData)
  ...
  async broadcastQueuedPaymentTransactions () {
    ...
    if (hasQueuedTransaction && !hasPendingTransaction) {
      try {
        ...
        console.log('broadcastQueuedPaymentTransactions[DEPRECATED]: nextQueuedTransactionData', nextQueuedTransactionData)
        // getNextQueuedTransaction()
        ...
        if (!successful_broadcast) {
          console.error('broadcastQueuedPaymentTransactions[DEPRECATED]: unsuccessful broadcast ! ')

          // this is putting in a bad entry !! like 'true '
          //   await this.redisInterface.pushToRedisList('queued_transactions',nextQueuedTransactionData)
        }
      } catch (e) {
        console.log('broadcastQueuedPaymentTransactions[DEPRECATED]: caught error', e)
      }
    }
  ...
  async broadcastTransaction (transactionPacketData) {
    ...
    console.log('\n')
    console.log('\n')
    console.log('broadcastTransaction: ---- broadcast transaction ---', txType, txData)
    ...
    if (txType == 'solution') {
      ...
      console.error('broadcastTransaction: invalid tx type!', txType)
      return false
    }
    ...
    if (tx_hash == null) {
      ...
      console.log('broadcastTransaction: broadcasted transaction -> ', tx_hash, txType, txData)
  ...
  async storeNewSubmittedTransferData (txHash, addressTo, balancePaymentId, tokenAmount) {
    ...
    console.log('storeNewSubmittedTransferData: Storing new submitted transfer data', ('balance_transfers:' + addressTo.toString()), balanceTransferData)
