...
module.exports = {
  ...
  async submitMiningSolutionTwo (minerAddress, solution_number, challenge_digest, challenge_number) {
    ...
    try {
      ...
      console.log('submitMiningSolutionTwo: estimateGas error', err)
      return
    }
    ...
    if (estimatedGasCost > max_gas_cost) {
      console.error('submitMiningSolutionTwo: Gas estimate too high!  Something went wrong ')
      return
    }
      ...
      this.sendSignedRawTransaction(this.web3, txOptions, addressFrom, privateKey, function (err, res) {
        if (err) {
          if (err.message.includes('insufficient funds for gas')) {
            console.error('submitMiningSolutionTwo: OUT OF GAS')
          } else {
            console.log('submitMiningSolutionTwo: sendSignedRawTransaction error', err)
          }
          error(err)
          return
        }
  ...
  */
  async checkBatchPaymentsStatus () {
    var self = this
    var unconfirmedBatches = await self.mongoInterface.findAll('payment_batch', { confirmed: false })
    console.log(`checkBatchPaymentsStatus: ${unconfirmedBatches.length} unconfirmed batches`)
    // this type of for loop works with async & .forEach does not
    for (var element of unconfirmedBatches) {
      var complete = await self.paymentContract.methods.paymentSuccessful(element.id).call()
      if (complete) {
        console.log(`checkBatchPaymentsStatus: batch payment ${element.id} complete!`)
