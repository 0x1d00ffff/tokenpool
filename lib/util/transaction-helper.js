...
/*
  ...
  async submitMiningSolutionWithHelper(minerAddress,solution_number,challenge_digest,challenge_number){
    ...
    console.log( '\n' )
    console.log( 'submitMiningSolutionWithHelper[DEPRECATED]: ---Submitting solution for reward---')
    console.log( 'submitMiningSolutionWithHelper[DEPRECATED]: nonce ',solution_number )
    console.log( 'submitMiningSolutionWithHelper[DEPRECATED]: challenge_number ',challenge_number )
    console.log( 'submitMiningSolutionWithHelper[DEPRECATED]: challenge_digest ',challenge_digest )
    console.log( '\n' )
  ...
  try{
    var txCount = await this.web3.eth.getTransactionCount(addressFrom);
    console.log('submitMiningSolutionWithHelper[DEPRECATED]: txCount',txCount)
   } catch(error) {  //here goes if someAsyncPromise() rejected}
    console.log('submitMiningSolutionWithHelper[DEPRECATED]: error',error);
   ...
   var addressTo = this.mintHelperContract.options.address;
    ...
    console.log('submitMiningSolutionWithHelper[DEPRECATED]: estimatedGasCost',estimatedGasCost);
    console.log('submitMiningSolutionWithHelper[DEPRECATED]: txData',txData);

    console.log('submitMiningSolutionWithHelper[DEPRECATED]: addressFrom',addressFrom);
    console.log('submitMiningSolutionWithHelper[DEPRECATED]: addressTo',addressTo);



    if( estimatedGasCost > max_gas_cost){
      console.log("submitMiningSolutionWithHelper[DEPRECATED]: Gas estimate too high!  Something went wrong ")
      return;
    }
  ...
  return new Promise(function (result,error) {
       ...
       this.sendSignedRawTransaction(this.web3,txOptions,addressFrom,privateKey, function(err, res) {
        if (err) {
		console.log('submitMiningSolutionWithHelper[DEPRECATED]: error', err);
		error(err);
	}
	console.log('submitMiningSolutionWithHelper[DEPRECATED]: got tx result', res);
          result(res)
      })
  ...
  async submitMiningSolutionOld(minerAddress,solution_number,challenge_digest,challenge_number,resending){
   ...
   console.log( '\n' )
   console.log( 'submitMiningSolutionOld: ---Submitting solution for reward---')
   console.log( 'submitMiningSolutionOld: nonce ',solution_number )
   console.log( 'submitMiningSolutionOld: challenge_number ',challenge_number )
   console.log( 'submitMiningSolutionOld: challenge_digest ',challenge_digest )
   console.log( '\n' )
  ...
  try{
   var txCount = await this.web3.eth.getTransactionCount(addressFrom);
   console.log('submitMiningSolutionOld: txCount',txCount)
  } catch(error) {  //here goes if someAsyncPromise() rejected}
   console.log('submitMiningSolutionOld: error',error);
  ...
  var addressTo = this.tokenContract.options.address;
   ...
   console.log('submitMiningSolutionOld: estimatedGasCost',estimatedGasCost);
   console.log('submitMiningSolutionOld: txData',txData);

   console.log('submitMiningSolutionOld: addressFrom',addressFrom);
   console.log('submitMiningSolutionOld: addressTo',addressTo);



   if( estimatedGasCost > max_gas_cost){
     console.log("submitMiningSolutionOld: Gas estimate too high!  Something went wrong ")
     return;
   }
  ...
  return new Promise(function (result,error) {
      ...
      this.sendSignedRawTransaction(this.web3,txOptions,addressFrom,privateKey, function(err, res) {
       if (err) {
 		console.log("submitMiningSolutionOld: sendSignedRawTransaction error", err);
	       error(err);
       }
         console.log("submitMiningSolutionOld: sendSignedRawTransaction got tx result", res);
         result(res)
     })
  ...
  async transferPaymentBatch(batchPayment)
  {
   ...
   {
     console.log('transferPaymentBatch: no payments in this batch')
     return
   }
   ...
   if(toValueArray.length == 0) {
     console.log('transferPaymentBatch: after processing, no payments in this batch');
     return;
   }
   ...
   var txData = this.web3.eth.abi.encodeFunctionCall({
	 ...
	 try {
		...
		//console.log("transferPaymentBatch: estimateGas threw an exception", err);
		throw err;
	 }
    ...
    console.log('checkBatchPaymentsStatus: done w for each ')
     ...
     return true ;
       ...
       async transferTokensFromPool(addressFromType, ethMinerAddress, amount, balancePaymentId, resending)
       {
            ...
            try{
              var txCount = await this.web3.eth.getTransactionCount(addressFrom);
              console.log('transferTokensFromPool[DEPRECATED]: txCount',txCount)
             } catch(error) {  //here goes if someAsyncPromise() rejected}
              console.log('transferTokensFromPool[DEPRECATED]: error', error);
             ...
             var txData = this.web3.eth.abi.encodeFunctionCall({
                 ...
                 var estimatedGasCost = await transferMethod.estimateGas({gas: max_gas_cost, from:addressFrom, to: addressTo });
                     ...
                     if( estimatedGasCost > max_gas_cost){
                       console.log('transferTokensFromPool[DEPRECATED]: Gas estimate too high! Something went wrong');
                       return;
                     }
                  ...
                  //   var privateKey =  this.getPaymentAccount().privateKey;
                   ...
                   return new Promise(function (result,error) {
                        ...
                        this.sendSignedRawTransaction(this.web3,txOptions,addressFrom,privateKey, function(err, res) {
                         if (err) {
                       		console.log('transferTokensFromPool[DEPRECATED]: sendSignedRawTransaction error', err);
				error(err);
			 }
                           result(res)
                       })
       ...
       {
         console.log('transferTokensFromPool[DEPRECATED]: requestCurrentChallengeNumber start')
         ...
         var result =  new Promise(function (fulfilled,error) {
           ...
           self.tokenContract.methods.getChallengeNumber().call(function(err, result){
              if(err){
                      console.log('transferTokensFromPool[DEPRECATED]: requestCurrentChallengeNumber error', err);
		      error(err);
		      return;
	      }
