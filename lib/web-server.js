...
module.exports = {
  ...
  async init (https_enabled, webInterface, peerInterface) {
    var server
    console.log('web-server:init: init web server...')
    ...
    if (https_enabled) {
      console.log('web-server:init: using https')
    ...
    app.listen(3000, () => console.log('web-server:init: Web app listening on port 3000!'))
  ...
  startSocketServer (server) {
    ...
    server.listen(port, function () {
      console.log('web-server:startSocketServer: Socket server listening at port %d', port)
    })

    var sockets = {}

    io.on('connection', function (socket) {
      console.log('web-server:startSocketServer: established new socket connection')

      socket.on('ping', function (data) {
        console.log('web-server:startSocketServer: ping', data)
      ...
      socket.on('getPoolStats', async function (data) {
        var result = await self.webInterface.getPoolStats()

        socket.emit('poolStats', result)
      })

      socket.on('getSubmittedShares', async function (data) {
        var result = await self.webInterface.getSubmittedShares(100)

        socket.emit('submittedShares', result)
      })

      socket.on('getSubmittedSolutions', async function (data) {
        var result = await self.webInterface.getSubmittedSolutions(100)

        socket.emit('submittedSolutions', result)
      })

      socket.on('getMinerDetails', async function (data) {
        var minerData = await self.peerInterface.getMinerData(data.address)

        var sharesData = await self.peerInterface.getSharesData(data.address)

        var allMinerData = minerData

        allMinerData.shareCredits = sharesData.shareCredits
        allMinerData.varDiff = sharesData.varDiff
      ...
      socket.on('disconnect', function () {
        console.log('web-server:startSocketServer:', socket.sid, 'disconnected')
